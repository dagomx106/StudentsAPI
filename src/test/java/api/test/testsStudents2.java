...
public class testsStudents2 {
	...
	void testPostStudent(ITestContext con) {
		...
		Response res = StudentEndPoints2.createStudent(this.studentPayload);
	...
	void testGetStudent(ITestContext con) {
		...
		Response res = StudentEndPoints2.getStudent(id);
	...
	void testPutStudent(ITestContext con) {
		...
		studentPayload.setPhone(fk.phoneNumber().cellPhone());
			logg.info("-----UPDATE----");
		Response res = StudentEndPoints2.updateStudent(this.studentPayload, (int)con.getSuite().getAttribute("id"));
		System.out.println(res.getBody().asPrettyString());
		System.out.println(res.jsonPath().getInt("id"));
		Assert.assertEquals(res.getStatusCode(), 200);
	...
	void testDeleteStudent(ITestContext con) {
			logg.info("-----DELETE----");
		Response res = StudentEndPoints2.deleteStudent((int)con.getSuite().getAttribute("id"));
